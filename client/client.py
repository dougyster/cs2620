...
class ClientApp:
    ...
    def login(self):
        ...
        # First response should be the bulk messages
        ...
        if chr(msg_type) == 'B':
            # Store the bulk messages temporarily
            self.pending_messages = payload
            
            # Now read the login success/failure message
            header = self.read_exact(5)
            if not header:
                messagebox.showerror("Error", "Connection closed")
                return
        ...
        if chr(msg_type) == 'S':
            ...
            self.chat_screen()  # Create chat screen first
            if hasattr(self, 'pending_messages'):  # Now handle the stored messages
                self.handle_bulk_messages(self.pending_messages)
